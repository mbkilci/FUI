...
const Trait = ({ heading, description }: TraitProps) => {
  return (
    ...
    >
      <VStack>
        <Text color={"white"} fontSize={"sm"}>{heading}</Text>
        <Text color={"white"} fontSize={"sm"} marginTop={"-2"} fontWeight={"semibold"}>
          {description}
        </Text>
      </VStack>
    </Box>
...
export default function Card({
  ...
  return (
    <Box backgroundColor={"#1f1f1f"} position={"relative"} width={"full"} overflow={"hidden"}>
      <Box
        key={image}
        height={"sm"}
        position="relative"
      ...
      />
      <Text color={"White"} fontWeight={"semibold"} marginTop={"15px"}>
        {metadata.name}
      </Text> 
